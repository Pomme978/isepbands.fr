...
const STORAGE_CATEGORY = 'inventory';
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB for inventory photos
const ALLOWED_TYPES = [
  'image/jpeg',
  'image/jpg',
  'image/png',
...
export async function POST(req: NextRequest) {
  ...
  // Only admins can upload inventory photos
  ...
  if (!isAdmin) {
    return NextResponse.json(
      { error: 'Forbidden - Only admins can upload inventory photos' },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    const imageUrl = `/api/storage/inventory?id=${dbFile.id}`;
...
export async function DELETE(req: NextRequest) {
  ...
  // Only admins can delete inventory photos
  const hasAdminRole = sessionUser.roles?.some((r: { role?: { name: string } }) =>
    ['president', 'vice_president', 'secretary', 'treasurer'].includes(r.role?.name),
  );
  const isAdmin = sessionUser.isFullAccess || sessionUser.isRoot || hasAdminRole;
  ...
  if (!isAdmin) {
    return NextResponse.json(
      { error: 'Forbidden - Only admins can delete inventory photos' },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    console.error('Error deleting inventory photo:', error);
    const lang = detectLangFromRequest(req);
    return NextResponse.json(
      { error: await getErrorMessage('fileNotFound', lang) },
      { status: 404 },
